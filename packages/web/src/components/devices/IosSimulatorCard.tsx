...
export default function IosSimulatorCard({ simulator }: { simulator: IosSimulator }) {
  ...
  const install = async () => {
    ...
    try {
      ...
      setFeedback({
        ok: false,
        message: err instanceof Error ? err.message : t('devices.ios.installFailed'),
      });
    } finally {
      setPending(false);
    }
  ...
  return (
    <div className="rounded-lg border border-zinc-800/80 bg-zinc-900/60">
      <div className="flex flex-wrap items-center gap-3 px-4 py-3">
        <span
          className="pulse-dot h-1.5 w-1.5 shrink-0 rounded-full bg-emerald-400"
          title={t('devices.ios.booted')}
        />
